  ...
  <div>
    <div class="index_more">
      ...
      <div class="index_chunk">
        <div class="t_nav">
           <router-link to="/recommend/officer/list">&#12288;我的推荐列表</router-link>
           <span> > 他的推荐列表 </span>
        </div>
        <hr>
...
import { ORGANIZ_DECLARER_RECOMMEND_PARENT_COUNT, ORGANIZ_DECLARER_RECOMMEND_PARENT } from '@/config/env';
...
export default {
  ...
  methods: {
    async search(page) {
      ...
      const res = await this.$http.get(ORGANIZ_DECLARER_RECOMMEND_PARENT, param);
      if (res.success) {
        this.lists = res.data || [];
      ...
      const res2 = await this.$http.get(ORGANIZ_DECLARER_RECOMMEND_PARENT_COUNT);
      if (res2.success) {
